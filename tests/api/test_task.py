...
def assert_task_response(data: dict, task: Task) -> None:
    ...
    assert data["starter_code"] == task.starter_code
...
def create_task_url() -> str:
    return str(app.url_path_for("create_task"))
...
def test_create_task_admin_uses_expected_url():
    assert create_task_url() == "/api/tasks/admin"
...
async def test_create_task_admin_success(
    ...
    assert data["starter_code"] == "print('start')"
    assert data["time_limit_ms"] == 2000
    assert data["memory_limit_mb"] == 256


@pytest.mark.asyncio
async def test_create_task_admin_allows_missing_starter_code(
    client,
    admin_headers,
    section_factory,
    db,
    ...
    assert response.status_code == status.HTTP_201_CREATED
    assert response.json()["starter_code"] is None


@pytest.mark.asyncio
async def test_create_task_admin_validates_starter_code(
    client,
    admin_headers,
    section_factory,
    db,
    ...
    response = await client.post(
        create_task_url(),
        headers=admin_headers,
        json={"step_id": step.id, "starter_code": ""},
    )

    assert response.status_code == status.HTTP_422_UNPROCESSABLE_CONTENT
    assert response.json()["detail"][0]["loc"] == ["body", "starter_code"]
...
@pytest.mark.asyncio
async def test_update_task_admin_updates_only_starter_code(
    client,
    admin_headers,
    section_factory,
    db,
    ...
    response = await client.patch(
        ...
        headers=admin_headers,
        json={"starter_code": "print('updated')"},
    )
    ...
    assert data["id"] == task.id
    assert data["step_id"] == task.step_id
    assert data["starter_code"] == "print('updated')"
    assert data["time_limit_ms"] == task.time_limit_ms
    assert data["memory_limit_mb"] == task.memory_limit_mb
    await db.refresh(task)
    assert task.starter_code == "print('updated')"
...
async def test_update_task_admin_rejects_non_admin(
    ...
    step = await create_step(db, section_factory)
    task = await create_task(db, step.id)
    original_starter_code = task.starter_code
    ...
    await db.refresh(task)
    assert task.starter_code == original_starter_code
